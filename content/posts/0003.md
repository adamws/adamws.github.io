...
using web-based deployment at [ergogen.xyz](https://ergogen.xyz/).
...
Now I can run *Generate* and download `points.yaml` file which will be used by kbplacer.

{{< callout warning >}}
New `ergogen` version may require enabling 'Debug' setting to generate `points.yaml`
{{< /callout >}}
...
{{< callout warning >}}
New `kbplacer` version use double dash (`--`) instead of single dash (`-`) for CLI arguments
{{< /callout >}}
